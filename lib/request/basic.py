...
def decodePage(page, contentEncoding, contentType):
    ...
    if not conf.encoding:
        ...
        if (any((httpCharset, metaCharset)) and not all((httpCharset, metaCharset))) or (
    ...
    if isinstance(page, six.binary_type) and "text/" in contentType:
        # e.g. &#x9;&#195;&#235;&#224;&#226;&#224;
        if "&#" in page:
            page = re.sub(r"&#x([0-9a-f]{1,2});",
                          lambda _: (_.group(1) if len(_.group(1)) == 2 else "0%s" % _.group(1)).decode("hex"), page)
        ...
        # e.g. &amp;
        page = re.sub(r"&([^;]+);",
                      lambda _: chr(htmlEntities[_.group(1)]) if htmlEntities.get(_.group(1), 256) < 256 else _.group(
                          0), page)
        ...
        if (kb.pageEncoding or "").lower() == "utf-8-sig":
            ...
            if page and page.startswith(
                    "\xef\xbb\xbf"):  # Reference: https://docs.python.org/2/library/codecs.html (Note: noticed problems when "utf-8-sig" is left to Python for handling)
                page = page[3:]
        ...
        # e.g. &zeta;
        page = re.sub(r"&([^;]+);",
                      lambda _: unichr(htmlEntities[_.group(1)]) if htmlEntities.get(_.group(1), 0) > 255 else _.group(
                          0), page)
